        ...
        if (document.readyState == "complete") {
            ...
            function pdata(data){
                ...
                for(let i=0;i<=jsObject.data.length;i++){
                    ...
                    console.log(task);
                    if(task.link){
                        const NTab=GM_openInTab(task.link+'?&clientver=12.44.9018&main_broker=WzEwMDFd&user_id_type=1&user_id=158049&channel=4&clientlang=0&clienttype=13&user_main_broker=WzFd&is_visitor=0&skintype=3', { active: false, insert: true, setParent :true });
                        setTimeout(() => {
                            NTab.close()
                        }, 5*1000);
                    }else{
